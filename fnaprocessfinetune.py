...
if __name__=='__main__':
    with open(f'E:\RNA_outputs/FinetuneplusUTR.csv','+w') as output_file:
        output_file.write(  ',' + "species" + ',' + "comment" + ',' + "5primeUTR" + ',' + "sequence" + ',' + "interval"+ ',' + "3primeUTR"+ ',' + "Score" + '\n')
        rna_set=set()
        cnt=0
        d_cnt=0
        # file_list=glob.glob(f'./{spec}/*/*.fna')
        file_list = glob.glob(f'./20240924好数据评分mRNA/*.fasta')
        file_list.extend(glob.glob(f'./20240924好数据评分mRNA/*.txt'))
                            ...
                            if len(slim_line) > 0 and len(utr5)>0 and len(utr3)>0:
                                if len(slim_line)>1 or len(slim_line[0])>6:
                                    ...
                                    if subline_all not in rna_set:
                                        ...
                                        # if cnt> 1000:
                                        #     break
                                        sco=os.path.split(file_name)[1].split(' ')[0]
                                        # print(sco)
                                        print("\rProcessing: {}, Processed line {}, sequence length {}".format(file_name,
                                                                                                             cnt,
                                                                                                             len(slim_line)),end='', flush=True)
                                        output_file.write(str(cnt)+ ','+ 'Unknown'+ ',' + last_name.replace(',','|')+ ','+ utr5+ ','+ cds_line.strip('—')+ ','+ inter_line.strip('—')+ ','+ utr3+ ','+ sco+ '\n')
                                    else:
                                        d_cnt+=1
